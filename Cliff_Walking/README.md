...
- **综合学习曲线图表**：使用Chart.js库在一张图表中展示完整的学习过程
  - 🔵 **Sarsa Episode Reward**：浅蓝色线显示Sarsa每回合的原始奖励数据
  - 🔴 **Sarsa Moving Average**：红色粗线显示Sarsa的移动平均趋势
  - 🟢 **Q-learning Episode Reward**：浅绿色线显示Q-learning每回合的原始奖励数据
  - 🟠 **Q-learning Moving Average**：橙色粗线显示Q-learning的移动平均趋势
- **可调节参数**：
  - 移动平均窗口大小（默认20回合）
  - 训练回合数设置（1-1000回合）
- **实时统计信息**：
  ...
  - 最新回合奖励显示
- **策略和价值可视化**：
  - 合并式网格显示：每个算法使用单一表格同时展示策略和状态价值
  - 策略箭头：每个单元格上方显示最优动作方向（↑↓←→）
  - 状态价值：每个单元格下方显示精确的状态价值数值
  - 水平对比布局：两个算法并排显示，便于直接对比
