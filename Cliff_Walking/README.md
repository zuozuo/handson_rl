...
- **学习率衰减过程可视化**：
  - 实时展示改进算法的alpha值衰减曲线
  - 对比固定学习率（红色虚线）与衰减学习率的差异
  - 验证自适应学习策略的有效性
- **收敛稳定性指标**：
  - 计算并展示Q值更新幅度的滑动平均
  - 通过更新幅度变化评估算法收敛过程的稳定性
  - 更新幅度趋于零表示算法已接近最优收敛状态
- **统计分析支持**：
  - 移动平均、标准差等统计指标计算
  - 滑动窗口分析平滑数据噪声
  - 多维度数据收集和实时可视化更新
...
## 时序差分算法可视化 (`temporal_difference_visualization.html`)

这是一个综合的交互式可视化平台，用于深入理解和对比时序差分算法的学习过程。

### 核心功能

#### 四算法对比实验
- **原始Sarsa算法**: 在线策略，使用固定的探索率(ε=0.1)和学习率(α=0.1)
- **原始Q-learning算法**: 离线策略，使用固定的探索率(ε=0.1)和学习率(α=0.1)
- **改进Sarsa算法**: 在线策略，探索率和学习率动态衰减(0.1→0.01，衰减率0.995)
- **改进Q-learning算法**: 离线策略，探索率和学习率动态衰减(0.1→0.01，衰减率0.995)

#### 多维度可视化
1. **策略可视化**: 显示每个算法学习到的最优动作选择
2. **状态价值可视化**: 显示每个状态的价值函数估计
3. **参数衰减可视化**: 实时显示改进算法的ε和α值变化
4. **最优策略追踪**: 记录算法达到最优策略的迭代步数
   - Sarsa算法最优标准：连续10回合奖励≥-18（考虑探索干扰的安全路径策略）
   - Q-learning算法最优标准：连续10回合奖励≥-16（考虑探索干扰的沿悬崖边缘策略）
   - 添加"重新检查最优策略"按钮，用于已训练算法的状态更新

#### 学习曲线分析
- **八条曲线对比**: 原始算法vs改进算法的回合奖励和移动平均对比
- **可调节移动平均窗口**: 平滑曲线便于观察收敛趋势
- **实时训练进度**: 显示当前训练回合数

#### 学习率与收敛性深度分析
1. **Q值收敛过程对比**: 监控特定状态Q值的收敛轨迹
2. **后期收敛细节**: 专注分析最后100回合的精细收敛过程
3. **学习率衰减过程**: 对比固定学习率vs动态衰减学习率
4. **收敛稳定性指标**: 分析Q值更新幅度的变化趋势

### 交互控制
- **统一训练控制**: 同时训练四个算法进行公平对比
- **单步训练模式**: 细粒度观察每回合的变化
- **一键重置**: 快速重新开始实验
- **图表清空**: 清理历史数据重新分析
- **防重复点击保护**: 训练期间自动禁用所有操作按钮，避免UI阻塞和重复执行
- **异步分批训练**: 大批次训练时分批执行，保持界面响应性
- **训练状态反馈**: 训练期间按钮文字显示"训练中..."状态

### 实验建议
1. **参数对比实验**: 先运行原始算法观察基准表现，再运行改进算法对比效果
2. **长期训练观察**: 建议设置500回合观察完整的收敛过程和参数衰减效果
3. **收敛性分析**: 关注改进算法是否能更快达到最优策略并保持稳定
4. **策略差异研究**: 对比Sarsa的保守策略vs Q-learning的激进策略

### 算法改进效果
改进算法通过参数衰减机制能够：
- **加速收敛**: 初期大学习率快速学习，后期小学习率精细调优
- **提高稳定性**: 降低的探索率减少后期的随机性干扰
- **更好的最优策略**: 通常能在更少的迭代步数内达到并维持最优策略 